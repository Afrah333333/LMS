...
import 'package:online_learning_platform/src/models/classses.dart';

class BuildClasses extends StatelessWidget {
  final DateFormat dateFormat = DateFormat("hh:mm a");

  BuildClasses({Key? key});
  ...
  _getTime(Classes c, context) {
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        border: Border.all(
          color: c.isPassed ? Colors.green : Colors.green,
        ),
      ),
  ...
  _getChild(Classes c, context) {
    if (c.isPassed) {
      return Icon(
        Icons.check,
        color: c.isPassed ? Colors.green.withOpacity(0.3) : Colors.green,
        size: 15.0,
      );
